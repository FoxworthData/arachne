...
class AiohttpFetcher:
    def __init__(
        ...
        response_analyzer: Optional[
            Callable[[aiohttp.ClientResponse, str, Dict[str, str], Logger, dict], Any]
        ] = None,
            ...
            try:
                async with session.get(
                    ...
                    if self.response_analyzer:
                        is_valid = await self.response_analyzer(response, url, headers, self.logger, self.store_identification)
                        if not is_valid:
